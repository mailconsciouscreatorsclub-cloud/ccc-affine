...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "[ERROR] Playwright not found in project" -ForegroundColor Red
...
if (Test-Path $browsersPath) {
    ...
    if ($browserCount -gt 0) {
        Write-Host "[OK] Found $browserCount browser(s) installed" -ForegroundColor Green
        Get-ChildItem $browsersPath -Directory | ForEach-Object {
            Write-Host "   - $($_.Name)" -ForegroundColor Gray
        }
    } else {
        Write-Host "[WARN] No browsers found" -ForegroundColor Yellow
    }
} else {
    Write-Host "[WARN] Browsers not installed yet" -ForegroundColor Yellow
}
...
if ($install -eq 'y' -or $install -eq 'Y') {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "[OK] Browsers installed successfully!" -ForegroundColor Green
    } else {
        Write-Host ""
    ...
    Write-Host "[SKIP] Skipping browser installation" -ForegroundColor Yellow
...
Write-Host "Step 4: Verifying MCP configuration..." -ForegroundColor Yellow
