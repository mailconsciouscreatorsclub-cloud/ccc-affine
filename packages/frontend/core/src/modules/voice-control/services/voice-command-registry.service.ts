...
export class VoiceCommandRegistry extends Service {
  ...
  private notifyChange(): void {
    for (const listener of this.changeListeners) {
      try {
        ...
        console.error('[voice-control] voice command listener failed', error);
      }
    }
  }
  ...
  private extractParameters(
    ...
    if (normalizedOriginal.startsWith(normalizedPhrase)) {
      remainder = normalizedOriginal.slice(normalizedPhrase.length).trimStart();
    }
    ...
    for (const definition of command.parameters) {
      ...
      if (value !== undefined) {
        ...
        return { parameters: {}, matchRatio: 0 };
